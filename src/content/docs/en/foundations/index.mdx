...
Verified against the local Supabase CLI stack: **Supabase CLI 2.117.0**, **Postgres 17** (`supabase/postgres:17.6.1`), **PostgREST** via the Data API, **`@supabase/supabase-js` v2**, and the **Deno 2** Edge Runtime — re-run `supabase --version` and `npm view @supabase/supabase-js version` for whatever is current when you read this.
