...
A channel can also be marked **private**, which restricts it to authenticated clients and routes authorization through a Row Level Security policy on the `realtime.messages` table — the same `using`/`with check` mechanism from the RLS module, just applied to who may join a channel rather than to rows in your own tables.
