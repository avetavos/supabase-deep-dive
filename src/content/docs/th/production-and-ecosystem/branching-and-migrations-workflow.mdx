...
description: 'Supabase branching สร้าง preview copy ของ database ที่ผูกกับ Git branch แยกต่างหากยังไง และ migration ไหลจาก branch ไปสู่ production ยังไง'
sidebar:
  order: 2
---
...
export const quizBranchingMigrations = [
  ...
  {
    q: 'ทำไมการรัน `supabase db push` ผ่าน CI ถึงสำคัญกว่าการรันเองด้วยมือเข้า production',
    options: [
      'ไม่สำคัญ เพราะทั้งสองวิธีปลอดภัยเท่ากัน',
      'การรันผ่าน CI จะ apply migration อัตโนมัติตามลำดับ เป็นส่วนหนึ่งของ pipeline เดียวกับที่ test บน branch มาแล้ว หลีกเลี่ยง ad hoc SQL และไฟล์ที่ลืมรันหรือผิดลำดับ',
      'CI จำเป็นเพราะ `supabase db push` รันจากเครื่อง local ไม่ได้เลย',
      'สำคัญเฉพาะ project ที่ไม่ได้ใช้ branching เท่านั้น',
    ],
    answer: 1,
  },
  {
