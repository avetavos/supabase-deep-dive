...
## อ่าน `explain analyze` ก่อนที่จะช้าไปแล้ว
...
export const quizPerformanceIndexing = [
  ...
  },
  {
    ...
    options: [
      'บอกจำนวน active user ที่ต่ออยู่ตอนนั้นแบบเป๊ะ ๆ',
      'โชว์ query plan จริงที่ Postgres เลือก เช่น sequential scan หรือ index scan พร้อมเวลาที่ใช้จริงและจำนวน row จริง',
      'สร้าง index ที่หายไปให้อัตโนมัติ',
      'ปิด Row Level Security ให้กับ query นั้นตัวเดียว',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      'ปิด RLS policy เพื่อให้ query เร็วขึ้นโดยไม่มีการเช็ค security',
      'ทำให้ Postgres กระโดดตรงไปยัง row ของ user คนนั้นแทนที่จะ scan ทั้ง table ทำให้ต้นทุน scale ตามข้อมูลของ user คนนั้นแทนที่จะเป็น size ของ table',
      'ทำสำเนา table todos ไปเป็น read-only replica ที่เร็วกว่า',
      'ไม่มีผลอะไรเลย เพราะ RLS บังคับให้เกิด sequential scan เสมอไม่ว่าจะมี index หรือไม่',
    ],
    answer: 1,
  },
];
