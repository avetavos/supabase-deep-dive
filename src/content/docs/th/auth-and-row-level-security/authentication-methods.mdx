...
Supabase Auth (**GoTrue**) รองรับ sign-in แบบ **password**, **magic link**, **phone OTP** และ **OAuth** มาให้เลย ไม่ว่า user จะเลือกวิธีไหน ผลลัพธ์เหมือนกันคือ row ที่ถูกสร้างหรือ match ใน **`auth.users`** — schema `auth` แยกต่างหากที่คุณต่อยอดด้วยการอ้างอิงจาก table **`public.profiles`** ของตัวเอง แทนที่จะไปแก้ `auth.users` ตรง ๆ
</Callout>
