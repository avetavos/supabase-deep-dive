...
security ของ Data API ใน Supabase มี **สองชั้น** ไม่ใช่ชั้นเดียว **Data API exposure** — ผ่านหน้า dashboard **Integrations → Data API** "Default privileges for new entities" หรือคำสั่ง `grant` ธรรมดาของ Postgres ให้ `anon`/`authenticated`/`service_role` — เป็นด่านระดับ table/function ที่ตัดสินว่า object นั้นเข้าถึงผ่าน API ได้ **เลยไหม** และเมื่อ request ผ่านด่านนั้นแล้วเท่านั้น **Row Level Security** ถึงจะตัดสินว่าเห็นหรือแก้ **row** ไหนได้บ้าง `service_role` มี attribute `BYPASSRLS` และข้าม RLS ไปทั้งหมด นี่คือเหตุผลที่ key ตัวนี้ต้องอยู่ฝั่ง server เท่านั้น
</Callout>
