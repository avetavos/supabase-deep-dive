...
สร้าง client ด้วย **`createClient`** และใช้ public **`anon` key** เสมอในโค้ดฝั่ง browser หรือ mobile — **RLS** ไม่ใช่ความลับของ key คือสิ่งที่ปกป้องข้อมูลจริง ส่วน **`service_role` key** bypass RLS และต้องอยู่ฝั่ง server เท่านั้น รัน **`supabase gen types typescript --project-id <ref> > database.types.ts`** แล้วเอา `Database` type ที่ได้ไปเป็น generic ให้ `createClient<Database>()` เพื่อได้ compile-time checking และ autocomplete บนทุก `.from()` call สร้าง client แค่ **ครั้งเดียว** แล้ว reuse instance เดียวกันทุกที่ เพราะเก็บ session state และจัดการ network connection ที่แชร์กันอยู่
</Callout>
...
export const quizJsClient = [
  ...
  },
  {
    ...
    options: [
      'ช้ากว่า anon key',
      'bypass row level security ทั้งหมด ถ้าหลุดออกไปใครก็อ่านหรือเขียน row ไหนก็ได้',
      'ใช้ได้แค่จาก Supabase CLI เท่านั้น',
      'หมดอายุภายในไม่กี่นาทีแล้วจะทำให้แอปพัง',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      'เก็บ session state ไว้ใน memory และจัดการ network connection ที่แชร์กัน สร้างใหม่ทุก request หรือ component จะเปลืองและอาจทำให้ auth state ไม่ sync กัน',
